/*****************************************************************
 * ST POS - DATA STORE / API (Google Apps Script)
 * ໜ້າເວັບ POS host ຢູ່ GitHub Pages. ໄຟລ໌ນີ້ເກັບຂໍ້ມູນລົງ Google Sheet ຢ່າງດຽວ.
 *   GET  ?callback=fn&token=...            -> JSONP { ok, state }   (ອ່ານເຕັມ — ຕ້ອງມີ token ຖ້າຕັ້ງໄວ້)
 *   GET  ?callback=fn&cust=1&table=A1      -> JSONP ຂໍ້ມູນສະເພາະໂຕະ (ລູກຄ້າ QR)
 *   GET  ?callback=fn&action=order&payload={...} -> ລູກຄ້າສັ່ງອາຫານ (server ກວດ PIN + ຕັດສະຕ໋ອກ)
 *   POST body = state JSON  (?token=...)   -> ບັນທຶກເຕັມ (ຕ້ອງມີ token ຖ້າຕັ້ງໄວ້)
 * ຂັ້ນຕອນ: Run seedDemo ເທື່ອທຳອິດ -> Authorize. Deploy: Web app, Anyone.
 * ແກ້ໂຄ້ດແລ້ວ: Deploy > Manage deployments > New version.
 *
 * ຄວາມປອດໄພ: ແກ້ຂໍ້ຄວາມໃນ setToken() ແລ້ວ Run setToken ເທື່ອດຽວ
 * ຈາກນັ້ນໃສ່ token ດຽວກັນໃນແອັບ (ການຕັ້ງຄ່າ → ການເຊື່ອມຕໍ່)
 *****************************************************************/
...
function doGet(e) {
  ...
  return ContentService.createTextOutput(JSON.stringify({ ok: true, app: 'ST POS data API' }))
...
/* ກັນບິນເສຍ ເມື່ອ 2 ເຄື່ອງບັນທຶກທັບກັນ */
function mergeBills_(curStr, inc) {
  try {
    ...
    cur.bills.forEach(function (b) {
      ...
      else if (m.ts !== b.ts || m.total !== b.total) {
...
function archiveNow_(days) {
  ...
  try {
    ...
    bills.forEach(function (b) { var creditOpen = b.credit && !b.creditSettled; if (b.ts >= cutoff || creditOpen) keep.push(b); else arch.push(b); });
    ...
    var rows = arch.map(function (b) { return [b.receipt, new Date(b.ts), b.table, b.total, b.pay, b.cashier, b.voided ? 'VOID' : '', (b.refundAmt || 0), (b.items || []).map(function (i) { return i.qty + 'x ' + i.name; }).join('; ')]; });
...
function putTable_(ss, name, header, rows) {
  var sh = tab_(ss, name); sh.clearContents();
  var data = [header]; for (var i = 0; i < rows.length; i++) data.push(rows[i]);
  sh.getRange(1, 1, data.length, header.length).setValues(data); sh.setFrozenRows(1);
}
...
function writeMirrors_(ss, st) {
  ...
  putTable_(ss, 'Settings', ['Item', 'Value'], [
    ...
    ['Floors', (st.floors || []).join(', ')], ['Receipt', st.receiptSeq || 0]
  ]);
  var tb = st.tables || {};
...
/* ແລ່ນ rebuild ເພື່ອສ້າງ/ອັບເດດ sheet ທັງໝົດ ຈາກຂໍ້ມູນປັດຈຸບັນທັນທີ */
function rebuild() {
  var ss = getSS_(); if (!ss) return 'no sheet';
  var s = getState_(); if (!s) return 'no data';
...
/* ແລ່ນເທື່ອທຳອິດ ເພື່ອສ້າງ Google Sheet + Authorize */
function seedDemo() {
  var demo = {
    shopName: 'ST POS', serviceChargePct: 0, vatPct: 0, billDiscPct: 0, qrGlobal: true, qrPinRequired: true, soundOn: true,
    activeShift: 'A', activeStaff: 'Staff', floors: ['F1', 'F2'], shifts: ['A', 'B'], staff: ['Staff'], receiptSeq: 10000,
    tables: { A1: { name: 'A1', floor: 'F1', status: 'free', customer: null, items: [] } },
    ingredients: {}, bills: [], orders: []
  };
  saveStateRaw_(JSON.stringify(demo));
  return 'OK - Sheet "ST POS DB" created';
}
